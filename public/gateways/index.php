...
$continent_methods = avilable_methods_continent_new($_PARAMS['country_id'],$dat->player_risk_level,$_PARAMS['web_id']);
...
if($dat->player_risk_level == 10){
	 if ($_PARAMS['web_id']==11) { // Mobile
        ...
        echo " <div> <h3 style='color:#fff;'>You dont have any payments methods. Please contact administrator.</h3>  </div>";
    }
...
if(empty($_VIRTUAL_PAYMENT) && $_PARAMS['is_first_time'] == 0){
    ...
    if($default_method){
         ...
         if($default_method->payment_method_id == 105){
		 	...
		 	$provider = 189;
		 }else if($default_method->payment_method_id == 133){
		 	$provider = 103;
		 }else if($default_method->payment_method_id == 134){
		 	$provider = 104;
		 }else if($default_method->payment_method_id == 135){
		 	$provider = 105;
		 }else if($default_method->payment_method_id == 142){
		 	$provider = 109;
		 }else if($default_method->payment_method_id == 144){
		 	$provider = 118;
		 }else if($default_method->payment_method_id == 143){
		 	$provider = 110;
		 }else if($default_method->payment_method_id == 145){
		 	$provider = 119;
		 }else{
		 	$provider = 108;
		 }
		...
		if($$active_methods[0]['payment_method_id'] == 105){
		 	...
		 	$provider = 189;
		 }else if($active_methods[0]['payment_method_id'] == 133){
		 	$provider = 103;
		 }else if($active_methods[0]['payment_method_id'] == 134){
		 	$provider = 104;
		 }else if($active_methods[0]['payment_method_id'] == 135){
		 	$provider = 105;
		 }else if($active_methods[0]['payment_method_id'] == 142){
		 	$provider = 109;
		 }else if($active_methods[0]['payment_method_id'] == 143){
		 	$provider = 110;
		 }else if($active_methods[0]['payment_method_id'] == 144){
		 	$provider = 118;
		 }else if($active_methods[0]['payment_method_id'] == 145){
		 	$provider = 119;
		 }else{
		 	$provider = 101;
		 }
...
<div id="deposit_main_div">
    ...
    <div class="deposit_leftdiv">
        ...
        <?php for($i=0;$i<count($payment_type);$i++){ ?>
            <?php if (!is_wap($_PARAMS['web_id'])){ ?>
                <div class="more <?php echo ($payment_type[$i]['id'] == $currentPaymentType->payment_type) ? 'active activedivid' : 'noactive' ?>"><h2>Deposit Options </h2></div>
            <?php } ?>


          <?php //echo $_PARAMS['is_dummy']; ?>

            <?php foreach($active_methods as $active): ?>
                <?php if($active['payment_type'] == $payment_type[$i]['id']){ 
                	if($active['payment_method_id']=='112' && $_PARAMS['is_dummy'] == 0){ $_PARAMS['is_dummy'] = 2;}?>
                    <div data-bound="<?php echo strtolower($active['name'])?>" data-id="<?php echo strtolower($active['payment_method_id'])?>" class="cuadro_metodos_boton_seleccion_metodo<?php echo ($_PARAMS['payment_method_id'] == $active['payment_method_id'] && ($_PARAMS['is_dummy'] == 0 || $payment_type[$i]['id'] == $_PARAMS['is_dummy'])) ? ' selected_method_btn' : ' unselected_method_btn' ?> featuvisacls <?php echo str_replace("/","",(str_replace(" ","",$payment_type[$i]['name']))) ?>" onclick="loadIframe('<?php echo strtolower($active['name'])?>', '<?php echo strtolower($active['payment_method_id'])?>', '<?php echo $active['payment_provider_id']?>', '<?php echo $payment_type[$i]['id'] ?>');" >
                        <img src="<?php echo $_PARAMS['payment_url']; ?>images/<?php echo $active['processor_logo'] ?>" width="86"  />
            ...
            <div style="width:100%;">
                ...
                <div style="width:100%;margin:auto;">
                    ...
                    <div class="contenido"<?php echo $styledisplay; ?>>
                        <?php if (is_wap($_PARAMS['web_id'])){ ?>
                        <div id="metolist_detalle_metodos" style="width:100%;margin-right:0px; margin-top:0px; min-height:340px;z-index:999;">
                            <?php }else{ ?>
                            <div id="metolist_detalle_metodos" style="width:100%;margin-right:0;float:left;min-height:340px;z-index:999;">
                                <?php } ?>
                                <div data-bound="visa">
                                    <noscript>
                                        <div class="tip">
                                        ...
                                        <div class="content" style="<?php echo $bar_color;?> /*border: 1px solid #999;*/ padding:0px;">
                                            ...
                                            <div class="title1">
                                                ...
                                                <div class="plogo" >
                                                    ...
                                                    <div class="bimg">
                                                        ...
                                                        <?php }else{  ?>
                                                        <img src="<?php echo $_PARAMS['payment_url']; ?>images/<?php echo $_PARAMS['image'] ?>" <?php echo $percentage_width; ?> <?php echo $ppercentage_height; ?>">
                                                        <?php } ?>
                                                    </div>
                                            ...
                                            switch($_PARAMS['payment_method_id']){
                                                ...
                                                case 111:
                                                    ...
                                                    break;
												case 112:
                                                    $provider_url = '../quickbits/form.php';
												...
												case 121:
													...
													break;
												case 122:
													$provider_url = '../netcents/form.php';
												...
												case 123:
													...
													break;
												case 133:
													$provider_url = '../qartpay/cardform.php';
													$provider_name = '/qartpay/';
													break;
													
												 case 134:
													$provider_url = '../qartpay/form.php';
													$provider_name = '/qartpay/';
													break;	
												 case 135:
													$provider_url = '../eazypay/form.php';
													$provider_name = '/eazypay/';
													break;		
												 case 142:
													$provider_url = '../hmtsquare/form.php';
													$provider_name = '/hmtsquare/';
													break;	
												case 144:
													$provider_url = '../payprohmt/form.php';
													$provider_name = '/payprohmt/';
													break;
												case 143:
													$provider_url = '../hmtsquare1/form.php';
													$provider_name = '/hmtsquare1/';
													break;	
												case 145:
													$provider_url = '../upiservice/form.php';
													$provider_name = '/upiservice/';
													break;				
                                            }
                                           ...
                                           // echo $provider_url; exit;
											include($provider_url);
											//include_once $provider_url;
                                            ?>
