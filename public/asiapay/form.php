...
define('PROVIDER_NAME', 'externalenglish');
...
if ($_REQUEST_ID){
   ...
   margin: auto;
	...
	if (VALIDATOR && $_PARAMS['amount'] && $_PARAMS['currency_id'] && $_PARAMS['payment_method_id'] && $_PARAMS['country_id'] && $_PARAMS['language']){
        ...
        $get_last_deposit_amount=isset($lastAmount->amount)?$lastAmount->amount/100:0;
		...
		<div class="main-container">
			<form id="idform" action='<?php echo $action; ?>' method='POST'>
				<div style="text-align: left;font-size: 15px;">Click on pay button to make deposit.</div>
				<div><p class="acc_h">Amount <span style="color:red;">*</span><span id="spaniput">
                        <input name="orderAmount" id="idorderAmount" value="<?php echo $amount; ?>" maxlength="6"  class="required" autocomplete="off" onclick="shownormalborders(this.id);" type="text">
                      </span>
                    </p>
					...
					<span style="font-size:10px;text-align: left; display:block;">*Multiple transactions allowed</span>
				</div>
