...
if ($_REQUEST_ID){
	...
	$_PARAMS['amount'] = $_POST['amount'];
	$_PARAMS['player_currency_amount'] = $_POST['amount'];
	//print('<pre>');print_r($_PARAMS);print_r($_POST);exit;
	...
	if ($_PARAMS['currency_id'] && $_PARAMS['payment_method_id'] && $_PARAMS['country_id'] && $_PARAMS['language']){
        ...
        if ($_PARAMS['player_currency_amount'] != ($_POST['amount'])){	
			...
			update_payments_new($_PAYMENT_ID,$_POST);
			
			echo "<p style='color: #000;font-family: sans-serif;background: #d6ffe2;display: inline-block;padding: 10px;border-radius: 4px;border: solid 1px #00ff49;font-weight: 600;'>Your Request Submitted successfully. Our team will update your payment status shortly</p>";
			exit;
			
			$callback_url= CALLBACKURL.'/hmtsquare/CallbackNotify.php?pay_id='.$_PAYMENT_ID;
			...
			$data['desc']='Test';
			$data['api_key']=$apikey;
			$data['merchant_order_token']='AKK'.$_PAYMENT_ID;
			$data['customer_email']=$email;
			$data['customer_mobile']=$contactno;
			$data['customer_first_name']=$name;
			$data['customer_last_name']=$lastname;
			$data['ord_title']='Product';
			$data['ord_image']='https://www.akkhacasino.com/assets/images/logo.png';
			$data['success_url']=$callback_url;
			$data['fail_url']=$callback_url;
			$data['pg_cancel_url']=$callback_url;
			//$data['payment_type']='Debit Card';
			//$data['mop_type']='VISA';
			...
			$headers = array(
			 ...
			 $logmessage = date('Y-m-d H:i:s')." hmtsquare request headers :".json_encode($headers)."\n";
			  file_put_contents($root_path.'/payments_hmtsquare.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			$response = json_decode($resp,true);	
			 ...
			 $logmessage = date('Y-m-d H:i:s')." hmtsquare response data :".json_encode($response)."\n";
			  file_put_contents($root_path.'/payments_hmtsquare.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);		
			//print_r($response);exit;
			if($response['status']==true || $response['status']=='true'){
				$order_token = $response['data'];
				update_payment_token($_PAYMENT_ID, $order_token);
				update_payment_foreignid($_PAYMENT_ID, $response['transaction_id']);
				...
				curl_close($ch);
				$response = json_decode($resp,true);
				$logmessage = date('Y-m-d H:i:s')." hmtsquare transactionid response data :".json_encode($response)."\n";
			  file_put_contents($root_path.'/payments_hmtsquare.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);		
				update_payment_foreignid($_PAYMENT_ID, $response['transactionId']);*/
