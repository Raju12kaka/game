   ...
   if($_PARAMS['bonuscode']!=''){
    ...
    <script language="javascript">
        ...
        function enteredAmountValue(playerId){
            ...
            // if(eAmount<= 100){
            //     $("#availbonusCode").addClass('required');
            // }else{
            //     $("#availbonusCode").removeClass('required');
            // }
        ...
        function checkform(f){
            ...
            <?php if($getCurrencyAvailableProcessor->count > 0) { ?>
                ...
                if (!f.orderAmount.value || !f.upi.value || f.orderAmount.value < parseInt(minamount)  || f.orderAmount.value > parseInt(maxamount) || (isNaN(f.orderAmount.value)) ){

            <?php } else { ?>	
                if (!f.convertedOrderAmount.value  || !f.upi.value || f.convertedOrderAmount.value < parseInt(minamount)  || f.convertedOrderAmount.value > parseInt(maxamount) || (isNaN(f.convertedOrderAmount.value)) ){
			<?php } ?>	
            ...
            /*var drpbonus = $('#availbonusCode').val();
            console.log("drop : "+drpbonus);
            console.log( "AMount : "+ f.orderAmount.value);
            
            if((f.orderAmount.value == 100) && (drpbonus ==0 || drpbonus =='' || drpbonus == undefined || drpbonus ==null)){
                showerrorborders('availbonusCode');
                return false;
            }else{
                shownormalborders('availbonusCode');
                return false;
            } */
        ...
        if(f.orderAmount.value == 100){
            if ($("#availbonusCode").val() == 0) {
                showerrorborders('availbonusCode');
                return false;
            }else{
                shownormalborders('availbonusCode');
...
<?php
    ...
    // echo $depositlimits->minimum_deposit_amount/100; die;
...
?>
		...
		<div class="content" style="">
            ...
            <p style="color:#fff;font-weight: bold;font-size:20px;margin-bottom:30px;"><span id="asterisc">*</span> Deposit with your UPI Address</p>
            <!-- <ol>
							...
							<td class="rowColoured">
						    	<div class="indent2">
                                    <div class="form_main_div">
                                        ...
                                        <div class="col-two">
                                            <div class="indent2" style="display:block;margin-bottom: 7%;padding-left: 0;"><b class="amount">Amount <span id="asterisc">*</span></b></div>
                                            <span style="font-size: 18px;margin-right: 0;display: inline-block;padding:10px 10px 9px;vertical-align: top;" class="indent2"><?php echo $currencysymbol['html_symbol']; ?></span>
                                            <input name="orderAmount" id="idorderAmount" value="<?php echo $amount; ?>" maxlength="6"  class="required" autocomplete="off" onclick="shownormalborders(this.id);" onkeyup="convertAmount('<?php echo $_PARAMS['player_currency_id'] ?>', '<?php echo DEFAULT_CURRENCY ?>', this.value); enteredAmountValue(<?php echo $_PARAMS['player_id']; ?>);" style="width:110px;padding: 13px;font-size: 14px;margin-left:-4px;margin-right:10px;border-radius:5px 5px 5px 5px;" type="text">
