...
include_once dirname(dirname(dirname(__FILE__))).'/models/define.php';
 ...
 $data =  json_encode($_REQUEST);
  ...
  $logmessage = date('Y-m-d H:i:s')." qartpay callback data :".$data."\n";
  ...
  $paymentid = $json_response_Data['ORDER_ID'];
  ...
  $statusInfo = $json_response_Data['RESPONSE_CODE']." || ".$json_response_Data['RESPONSE_MESSAGE'];
  $orderInfo = $json_response_Data['STATUS'];
  
  	if($json_response_Data['RESPONSE_CODE'] == '000'){
	...
	if($payment_details->status != 'SUCCESS'){
		update_payments_status($json_response_Data['ORDER_ID'], $player->mstrid, $status, $_RESULT['html'], "", $charges, $amount, $totalamount);
		if(!empty($json_response_Data['TXN_ID']))
			update_payment_foreignid($json_response_Data['ORDER_ID'], $json_response_Data['TXN_ID']);
		if($json_response_Data['RESPONSE_CODE'] == '000'){
			...
			/*postback aff*/
			if($player->affiliate_id==21){
				
				$trans_id = $player->aff_trans_id;
				$reference_number = 'AKD'.'123'.$json_response_Data['ORDER_ID'].'789';
				$posturl = "https://tracking.icubeswire.co/aff_a?offer_id=2458&goal_id=3574&transaction_id=".$trans_id."&adv_sub1=".$reference_number."&adv_sub2=";
