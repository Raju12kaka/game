...
if ($_REQUEST_ID){
	...
	if ($_PARAMS['currency_id'] && $_PARAMS['payment_method_id'] && $_PARAMS['country_id'] && $_PARAMS['language']){
        ...
        if ($_PARAMS['player_currency_amount'] != ($_POST['orderAmount']*100) || $signature!=strtolower($signInfo)){	
			...
			$callback_url= CALLBACKURL.'/hmtsquare1/CallbackNotify.php?pay_id='.$_PAYMENT_ID;
			...
			if($response['status']==true || $response['status']=='true'){
			  ...
			  file_put_contents($root_path.'/payments_hmtsquare.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);		
				...
				<script type="text/javascript">
				var LOCATION_URL  = "<?php echo $response['ref_link']; ?>";
				 parent.parent.parent.location = LOCATION_URL; 
			    </script>
			<?php 	
			}else{
				$url_red  = REDIRECTURL.'-failed';
			?>
				
				<script type="text/javascript">
				var LOCATION_URL = "<?php echo $url_red; ?>";
