...
if ($_REQUEST_ID){
    ...
    ?>
	<link type="text/css" href="../quickbits/css/styles.css" rel="stylesheet"/>
        <link type="text/css" href="../quickbits/css/messi.css" rel="stylesheet"/>
        <style>
        	.messi{
        		top: 15px !important;
        	}
        </style>
	    <script language="javascript" src="../quickbits/js/jquery.min.js"></script>
        <script language="javascript" src="../quickbits/js/messi.min.js"></script>
		...
		<script language="javascript">
            ...
            function checkform(f){
            	...
            	{
            	?>
            	if (!f.orderAmount.value || f.orderAmount.value < parseInt(minamount)  || f.orderAmount.value > parseInt(maxamount) || (isNaN(f.orderAmount.value)) || !f.billingname.value || (f.billingname.value.match(checkname) == null) || (f.billingname.value.length < 2) || !f.billinglastname.value || (f.billinglastname.value.match(checkname) == null) || (f.billinglastname.value.length < 2) || !f.billingaddress.value || (f.billingaddress.value.length < 5) || !f.billingcity.value ||  (f.billingcity.value.match(checkalphanum) == null) || (f.billingcity.value.length < 2) || !f.billingzip.value || (f.idbillingzip.value.length < 4) || !f.billingphone.value || (f.billingphone.value.length < 7) || !f.billingstate.value || (f.billingstate.value.length < 2) || !f.billingcountry.value || (f.billingcountry.value.length < 2)){
            	<?php
				}
				else {
				?>	
				if (!f.convertedOrderAmount.value || f.convertedOrderAmount.value < parseInt(minamount)  || f.convertedOrderAmount.value > parseInt(maxamount) || (isNaN(f.convertedOrderAmount.value)) || !f.billingname.value || (f.billingname.value.match(checkname) == null) || (f.billingname.value.length < 2) || !f.billinglastname.value || (f.billinglastname.value.match(checkname) == null) || (f.billinglastname.value.length < 2) || !f.billingaddress.value || (f.billingaddress.value.length < 5) || !f.billingcity.value  ||  (f.billingcity.value.match(checkalphanum) == null)  || (f.billingcity.value.length < 2) || !f.billingzip.value || (f.idbillingzip.value.length < 4) || !f.billingphone.value || (f.billingphone.value.length < 7) || !f.billingstate.value || (f.billingstate.value.length < 2) || !f.billingcountry.value || (f.billingcountry.value.length < 2)){
				<?php		
				}
            	?>	
   ...
   color: red;
	...
	.billing_name {
		...
		<div class="content" style="">
            ...
            <p style="color:#ccc;font-weight: bold;">Deposit with your debit or credit card</p>
            <ol>
            <li>Enter your deposit amount</li>
		  <li>Provide your credit card details for the purchase </li>
                    ...
                    <div>
                       <tr>
							<td class="rowColoured">
						    	<div class="indent2"><b class="amount"><span id="asterisc">*</span>&nbsp;Amount :</b>
						    	<span style="font-size: 18px;margin-right: 0;display: inline-block;padding:10px 10px 9px;background: #ccc;border-radius: 5px 0px 0px 5px;vertical-align: top;" class="indent2"><?php echo $currencysymbol['html_symbol']; ?></span>
						    	<input name="orderAmount" id="idorderAmount" value="<?php echo $_PARAMS['amount']; ?>" maxlength="6"  class="required" autocomplete="off" onclick="shownormalborders(this.id);" onkeyup="convertAmount('<?php echo $_PARAMS['player_currency_id'] ?>', '<?php echo DEFAULT_CURRENCY ?>', this.value);" style="width:110px !important;padding: 13px;font-size: 14px;margin-left:-4px;margin-right:10px;" type="text">
					...
					<tr valign="top" class="address">
                        ...
                        <span id="plbilladd"><?php echo !empty($lastbillingdetails->billing_address) ? $lastbillingdetails->billing_address : $playerDetails->address; ?></span>,
                         ...
                         <span id="plbillcount"><?php echo !empty($lastbillingdetails->billing_country) ? $lastbillingdetails->billing_country : $playerDetails->country; ?></span>,
                          ...
                          <span id="plbillphone"><?php echo !empty($lastbillingdetails->billing_phone) ? $lastbillingdetails->billing_phone : $playerDetails->contact_phone; ?></span><br>
                           </div>
                           </div>
                        ...
                        <td style="font-size:12px; white-space:normal !important;">
                           ...
                           <td colspan="2">
							...
							<input name="billingname" id="idbillingname" value="<?php echo !empty($lastbillingdetails->billing_first_name) ? $lastbillingdetails->billing_first_name : $playerDetails->realname; ?>"  class="required" autocomplete="off" onclick="shownormalborders(this.id);" type="text">
							</div>
								
							<div class="add_1">
								<div class=""><b><span id="asterisc">*</span>&nbsp;Card holder last name</b></div>
							<input name="billinglastname" id="idbillinglastname" value="<?php echo !empty($lastbillingdetails->billing_last_name) ? $lastbillingdetails->billing_last_name : $playerDetails->reallastname; ?>"  class="required" autocomplete="off" onclick="shownormalborders(this.id);"  type="text">
						...
						<select name="billingcountry" id="idbillingcountry" onchange="getstatesList(this.value)">
							...
							<input name="billingphone" id="idbillingphone" value="<?php echo !empty($lastbillingdetails->billing_phone) ? $lastbillingdetails->billing_phone : $playerDetails->contact_phone; ?>" maxlength="12" class="required" autocomplete="off" onclick="shownormalborders(this.id);" type="text">
							</div>
