	...
	if ($_REQUEST_ID){
		...
		if (VALIDATOR && $_PARAMS['amount'] && $_PARAMS['currency_id'] && $_PARAMS['payment_method_id'] && $_PARAMS['country_id'] && $_PARAMS['language']){
			...
			**********************************************************************/
			...
			'orderid'=>"SC".$orderNo,  //'YOUR ORDER ID'  
			'transaction_type'=>'a', 
			//'signature'=>$signature, 
			'amount'=>$orderAmount,  
			'currency'=>$currency,  
...
// 			$jsonresponse = $pay->payment();
			...
			$params['signature'] = $pay->getSignature($params, $sec_key);
			$params['transaction_hash'] = $_POST['transaction_hash'];
			//echo "<pre>"; print_r($params); exit;
			$curlRequestURL		= 'https://pay.paymentechnologies.co.uk/authorize_payment';
			...
			curl_setopt($curl,CURLOPT_POSTFIELDS,$RequestData);//Transmit datas by using POST
			//curl_setopt($curl,CURLOPT_REFERER,$returnUrl);
			$jsonresponse = curl_exec($curl);
			curl_close ($curl);
// 			
			parse_str($jsonresponse, $finalResult);
