      ...
      $arr = array(
       ...
       $url  = "https://services.avenuepay.com/apinterface"; 
		   ...
		   if(isset($result['redirectUrl']) && !empty($result['redirectUrl'])){
		   	file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')." ccpay3d transaction redirect url: ".$result['redirectUrl'].PHP_EOL , FILE_APPEND | LOCK_EX);
		   ?>
		   <script language="javascript">
			parent.parent.location="<?php echo $result['redirectUrl']; ?>";
			</script>
		   <?php
		   exit;
		   } 
            ...
            $orderInfo    = $result['reason'];//return orderInfo
            $riskInfo     = $result['reason'];//return riskInfo
