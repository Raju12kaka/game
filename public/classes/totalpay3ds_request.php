 ...
 **********************************************************************/
     ...
     ****************************/
     ...
     // $url = "https://secure.totalpay.co/rest/v1/transactions/".$arr['ext_order_id']."?Authorization=".base64_encode($key);
	   ...
	   file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')."TOTALPAY3DS::REQUEST ::".json_encode($arr).PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			curl_setopt($curl, CURLOPT_HTTPHEADER, array('Authorization: Basic '.base64_encode($key), 'Content-Type: application/json')); 
			$response = curl_exec($curl); 
		    file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')."TOTALPAY3DS::RESPONSE ::".$response.PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			$url = "http://secure.totalpay.co/rest/v1/transactions/".$arr['ext_order_id']."?Authorization=".base64_encode($key);  
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_URL, $url);
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			$response1 = curl_exec($curl); 
			file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')."TOTALPAY3DS after sleep::RESPONSE ::".$response1.PHP_EOL , FILE_APPEND | LOCK_EX);
			
			$result_response1 = json_decode($response, true);
