 ...
 **********************************************************************/
      ...
      $merchant_auth_code 	= trim($_POST['authorisationkey2']); //terminalname.
      $url                      = trim($_POST['authorisationkey3']); //URL qwipi
	  ...
	  $birthdate		= !empty($palyer_details->birthdate) ? date('Ymd', strtotime($palyer_details->birthdate)) : "19751023";
      $currency = "USD";
	...
	$md5Info         =  md5($merNo.$orderNo.$currency.$orderAmount.$date_time.$merchant_auth_code);
	...
	/****************************** 
      ...
      /*$arr = array(
             ...
             //'md5Key'        => $signkey,        //signkey.
             'billNo'          => $orderNo,          //OrderNo.
             'currency'    => $orderCurrency,    //OrderCurrency
             'amount'      => $orderAmount,      //OrderAmount
             'firstName'        => $firstName,        //FirstName
             'lastName'         => $lastName,         //lastName
             'cardNum'           => $cardNo,           //CardNo
             'month'  => $cardExpireMonth,  //CardExpireMonth
             'year'   => $cardExpireYear,   //CardExpireYear
             'cvv2' => $cardSecurityCode, //CVV
       	...
       	$post_data = "resType=XML&merNo=".$merNo."&billNo=".$orderNo."&currency=".$currency."&amount=".$orderAmount."&firstName=".$firstName."&lastName=".$lastName."&cardNum=".$cardNo."&month=".$cardExpireMonth."&year=".$cardExpireYear."&cvv2=".$cardSecurityCode."&email=".$email."&cardHolderIp=".$ip."&dob=".$birthdate."&phone=".$phone."&country=".$country."&state=".$state."&city=".$city."&address=".$address."&zipCode=".$zip."&language=ENG&dateTime=".$date_time."&md5Info=".$md5Info."&userAgent=".$useragent;
       //$datapost =  http_build_query($arr);
       ...
       //$url  = "https://services.avenuepay.com/apinterface"; 
	    ...
	    $curl = curl_init($url);
            ...
            //curl_setopt($curl,CURLOPT_POSTFIELDS,$arr);//Transmit datas by using POST
	...
	if (curl_error($curl)) {
	    ...
	    $result['merNo']        = (string)$xmlob->merNo; //return merNo    
            $result['tradeNo']      = (string)$xmlob->orderId;//return tradeNo
            $result['orderNo']      = $orderNo;//return orderno
            $result['orderAmount']  = (string)$xmlob->amount;//return orderAmount
            $result['orderCurrency'] = (string)$xmlob->currency;//return orderCurrency
                        $result['dateTime'] =    (string)$xmlob->dateTime;//return orderCurrency
            $result['resultCode']  = (string)$xmlob->resultCode;//return orderStatus
            $result['orderInfo']    = (string)$xmlob->billingDescriptor;//return orderInfo
            $result['signInfo']     = (string)$xmlob->md5Info;//return signInfo
            $result['riskInfo']     = (string)$xmlob->remark.$post_data.$xmlrs.$url;//return riskInfo
            $result['errorCode']     = (string)$xmlob->errorCode;//return riskInfo


            //signInfocheck
            $result['signInfocheck']=md5($result['merNo'].$result['orderNo'].$result['orderCurrency'].$result['orderAmount'].$result['resultCode'].$result['dateTime'].$signkey);
		...
		if(isset($error_msg)){
                        file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')."QWIPI::CURL ERROR ::".$error_msg."::REQUEST::".$post_data.PHP_EOL , FILE_APPEND | LOCK_EX);
		}
		else
		{
			file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')."QWIPI::REQUEST AND RESPONSE::".$result['riskInfo'].PHP_EOL , FILE_APPEND | LOCK_EX);
		}
            ...
            $orderStatus  = $result['resultCode'];//return orderStatus
            $orderInfo    = $result['orderInfo'];//return orderInfo
            $riskInfo     = $result['riskInfo'];//return riskInfo
