	...
	if ($_REQUEST_ID){
		...
		if (VALIDATOR && $_PARAMS['amount'] && $_PARAMS['currency_id'] && $_PARAMS['payment_method_id'] && $_PARAMS['country_id'] && $_PARAMS['language']){
			...
			****************************/
			...
			$signature = $merName."SE".$orderNo.$currency.$orderamount.$firstName.$lastName.$cardNo.$cardExpireYear.$cardExpireMonth.$cardSecurityCode.$email.$apiKey;
			$signature = hash('sha256', $signature);
			
			$logmessage 	= date('Y-m-d H:i:s')." Fstdebitpay Signature create :".$merName."SE".$orderNo.$currency.$orderamount.$firstName.$lastName.$cardNo.$cardExpireYear.$cardExpireMonth.$cardSecurityCode.$email.$apiKey."\n";
			file_put_contents($root_path.'/payments_fastdebitpay.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);
			
			$logmessage 	= date('Y-m-d H:i:s')." Fstdebitpay Signature :".$signature."\n";
			file_put_contents($root_path.'/payments_fastdebitpay.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			$requestArr = array(
				...
				'order_number'		=> "SE".$orderNo,			//OrderNo.
			...
			$logmessage 	= date('Y-m-d H:i:s')." payofix request data :".http_build_query($requestArr)."\n";
			file_put_contents($root_path.'/payments_fastdebitpay.log', $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			if(isset($_POST['submitfrom']) && $_POST['submitfrom']=="cron"){
			 ...
			 $result['tradeNo']      = $finalResult['transaction_id'];//return tradeNo
             ...
             $result['orderStatus']  = $finalResult['result'];//return orderStatus
             $result['orderInfo']     = $finalResult['message'];//return riskInfo
             file_put_contents($root_path.'/payments.log', date('Y-m-d H:i:s')." payofix response data for cron: ".json_encode($result).PHP_EOL , FILE_APPEND | LOCK_EX);
			...
			/*$resp_status = $finalResult['success'];
			$resp_message = $finalResult['message'];
			$resp_result = $finalResult['result']; //if result 1-approved, 2-failed, 0-processing
			$resp_transaction_id = $finalResult['transaction_id']; //Payofix transactionId
			$resp_descriptor = $finalResult['descriptor'];*/
			...
			if (isset($finalResult)){
				...
				else
				{
					...
					//$checkCountryEnable = get_country_dummy_status($_PARAMS['country_id'], $_PARAMS['payment_method_id']);
					/*
