...
class DummyProcess{
	...
	function sendPaymentToGateway($providers, $depositDetails){
		...
		$paymentproviderid=$providers['id'];
		    ...
		    if($providers['id'] =='183' || $providers['id'] =='184'){
		     ...
		     $form['deviceNo'] = '10a43aca5b699ab992b6584aca3a48b4';
			}
		     ...
		     // $form['direct_csid'] ='<script>var vals = document.getElementById("csid");document.write(vals.value);</script>';
			  $form['direct_csid'] = 'EN-US&@EN-US&@MOZILLA&@&@NETSCAPE&@5.0 (WINDOWS NT 10.0; WIN64; X64) APPLEWEBKIT/537.36 (KHTML, LIKE GECKO) CHROME/78.0.3904.97 SAFARI/537.36&@TRUE&@A36D8AE51CC0B0E2947AA2A881AFFF521573635221775OLD&@9498330UNDEFINED&@WIN32&@MOZILLA/5.0 (WINDOWS NT 10.0; WIN64; X64) APPLEWEBKIT/537.36 (KHTML, LIKE GECKO) CHROME/78.0.3904.97 SAFARI/537.36&@&@UNDEFINED&@1366X768&@24&@5.5&@PAYMENTS.SLOTS7CASINO.LOCAL&@3F95CE8919CC7CD9ADE8F521E43A48391573817578649';
			 }
			...
			file_put_contents($this->root_path, $logmessage.PHP_EOL , FILE_APPEND | LOCK_EX);
             ...
             $result['orderStatus']  = $result1['orderStatus'];//return orderStatus
			 ...
			 if($orderStatus == "1" ){
				$this->updatePayments($providers['method_id'], $providers['id'], $depositDetails['id']);
	...
	function updatePayments($methodId, $providerId, $paymentId){
		$dbConn = $this->connection_DB();
		if(!$dbConn){
			return false;
		}
		
		$SQL  = "UPDATE payments set payment_method_id = ? ,payment_provider_id = ? WHERE id = ? ";
		try{
			$res = $dbConn->prepare($SQL);
			$res->execute(array($methodId, $providerId, $paymentId));
		}catch(PDOException $ex) {
	        return false;
	    }
	}
